...
int ffp_start_record(FFPlayer *ffp, const char *file_name)
{
    ...
    VideoState *is = ffp->is;
    if (is == NULL) {
        av_log(ffp, AV_LOG_ERROR, "VideoState *is == NULL");
        return -1;
    }
    ffp->m_ofmt_ctx = NULL;
    ffp->m_ofmt = NULL;
    ...
    for (int i = 0; i < is->ic->nb_streams; i++) {
        ...
        if (in_stream == NULL) {
            av_log(ffp, AV_LOG_ERROR, "AVStream *in_stream == NULL");
            continue;
        }
...
int ffp_record_file(FFPlayer *ffp, AVPacket *packet)
{
    ...
    if (ffp->is_record) {
        ...
        if (pkt == NULL) {
            av_log(ffp, AV_LOG_ERROR, "AVPacket *pkt == NULL");
            return 0;
        }
        av_new_packet(pkt, 0);
        //解决缓存为null导致崩溃的问题
        if (packet->buf->size > 2 &&0 == av_packet_ref(pkt, packet)) {
            ...
            if (!ffp->is_first) {
                ...
                if (pkt->stream_index == AVMEDIA_TYPE_VIDEO && (pkt->flags & AV_PKT_FLAG_KEY)) {
                    ...
                    av_log(ffp, AV_LOG_ERROR, "录制：第一帧");
                } else {
                    av_packet_unref(pkt);
                    pthread_mutex_unlock(&ffp->record_mutex);
                    ffp->record_starting = 0;
